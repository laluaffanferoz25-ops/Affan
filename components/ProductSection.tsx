...
const ProductSection: React.FC<Props> = ({ product, reverse, onViewDetails }) => {
  return (
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center px-10 md:px-24 py-16 md:py-0">
        ...
        <div className="space-y-6">
          ...
          <div className="pt-8 flex flex-col md:flex-row items-center gap-4">
            <div className="text-2xl font-serif md:mr-10 mb-4 md:mb-0">{product.price}</div>
            <button 
              onClick={() => onViewDetails(product)}
              className="w-full md:w-auto border border-white/40 hover:bg-white hover:text-black transition-all px-12 py-4 text-[10px] uppercase tracking-[0.3em]"
            >
              View Details
            </button>
          </div>
